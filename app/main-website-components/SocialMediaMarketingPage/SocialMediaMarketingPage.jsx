...
import { ArrowUpRight, BarChart3, CalendarDays, MessageCircle, Target } from "lucide-react";
...
export default function SocialMediaMarketingPage() {
  return (
    <div className="cap-page growth-page social-page">
      ...
      <section className="social-engine"><div className="cap-shell"><header><span className="cap-kicker">The content engine</span><h2 className="cap-heading">Social growth works when creative and operations move together.</h2></header><div className="social-engine__grid" role="list">{engine.map(({title,text,icon:Icon},index)=><article key={title} role="listitem"><span className="social-engine__icon" aria-hidden="true"><Icon size={17}/></span><span className="social-engine__num" aria-hidden="true">0{index+1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>
